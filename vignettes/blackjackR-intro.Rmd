...
```{r}
# Normally the game will require input from the user to continue, therefore fake inputs are needed for the game to run here
# Fake inputs will be alway stand
...
Simulates a full round for multiple players vs the dealer, the game can play with unlimited number of players, but here we will use 2 players as an example:

```{r}
# Normally the game will require input from the user to continue, therefore fake inputs are needed for the game to run here
# Fake inputs will be alway stand
play_blackjack_multi(2, input_fn = function(prompt) "s")
```
